...
167. Recapped Game Programming, Module I - I troduction, related to different roles loke Game Designer, Programmer, Artist, Writer, Tester, game genres.
168. Learned Module XIV - 3D Animation in Game Development - I learned the basics of animation in Maya from rigging, skinning, and layering animation to make characters move in a believeable way, and the importance to simplify animation curves to optimize your game.
169. Finished Module XV - Integrating Assets with Animations in Game Engines - this one was a bit tricky; it explained about animation importing, blend trees, layering, animation optimization through caching, pretty cool. As a porgrammer, I thought animation is much more simple than that, but after the experence with Squash the Creeps I must say that animation is really complex:
170. Finished Module XVI - Engine Best Practices - This one is really cool and useful. I learned different modes to optimize the game, either through Rendering (Level-Of-Detail, Occlusion Culling), GPU (GPU Instancing and Shader Optimization), Memory (Asset Streaming) and Build and Deployment optimization (selecting the target platform, script backend either Manual or IL2CPP and optimization level), Audio (Audio Compression), and lastly CPU (Multithreading).
